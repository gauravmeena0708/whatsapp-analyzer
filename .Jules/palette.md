...
## 2024-05-18 - Explaining disabled states and safe error handling
**Learning:** Disabled interactive elements (buttons, select dropdowns) leave users guessing why they cannot interact with them. Providing a native `title` attribute explicitly states the prerequisite (e.g., "Please select a chat file"). Additionally, asynchronous operations using `FileReader` can fail, and their `onerror` and `catch` blocks must carefully restore or maintain disabled states rather than blindly re-enabling them, preventing broken UI states.
**Action:** Always add native `title` attributes to disabled interactive elements to explain their state, toggle these titles using JavaScript when states change, and review error handlers to ensure they correctly re-evaluate the interactive surface based on whether preconditions (like successfully reading a file) were met.

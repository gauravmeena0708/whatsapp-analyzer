...
class TestAnalysisUtils(unittest.TestCase):
    ...
    def test_basic_stats_no_emojis_user(self):
        """Test basic_stats for a user with messages containing no emojis."""
        chat_lines = [
            "11/01/23, 10:00 AM - UserNoEmoji: Just plain text.",
            "11/01/23, 10:05 AM - UserNoEmoji: Another message without any fancy symbols."
        ]
        clean_df = self._create_and_parse_chat_file(chat_lines)
        
        # Ensure 'UserNoEmoji' exists in the parsed data
        self.assertTrue("UserNoEmoji" in clean_df['name'].unique(), "UserNoEmoji should be in the DataFrame")

        stats_dict = basic_stats(clean_df.copy(), username='UserNoEmoji')

        # Check that the 'Emoji Usage' plot (base64 string) exists and is non-empty
        self.assertIn('Emoji Usage', stats_dict)
        self.assertIsInstance(stats_dict['Emoji Usage'], str)
        self.assertTrue(len(stats_dict['Emoji Usage']) > 0, "Emoji Usage base64 string should not be empty.")

        # Check that 'Top 5 Emojis' is an empty list
        self.assertIn('Top 5 Emojis', stats_dict)
        self.assertIsInstance(stats_dict['Top 5 Emojis'], list)
        self.assertEqual(len(stats_dict['Top 5 Emojis']), 0, "Top 5 Emojis list should be empty.")
